...
public class Graph extends JFrame {
    ...
    public void loadGraph() {
        ...
        for (Map.Entry<String, DepoVertex> dv : depoVertices.entrySet()) {
            for (Map.Entry<String, DepoConnection> pipe : simulation.getDepoConnections().entrySet()) {
                addPipe(pipe.getKey(), dv.getValue().getX(), dv.getValue().getY(),
                        getDepoVertex(pipe.getValue().getLeftDepoID()).getX(),
                        getDepoVertex(pipe.getValue().getLeftDepoID()).getY(),
                        pipe.getValue().getPipeDiameter());
            }
        }
    }
    ...
    public void paint(Graphics g) {
        ...
        for (Map.Entry<String, DepoConnection> dc : simulation.getDepoConnections().entrySet()) {
            ...
            DepoVertex start = getDepoVertex(dc.getValue().getLeftDepoID());
            DepoVertex end = getDepoVertex(dc.getValue().getRightDepoID());
            int diameter = dc.getValue().getPipeLength();
            for(Map.Entry<Integer, List<Double>> pos : positions.entrySet()) {
                int fuelID = pos.getKey();
                drawFuel(g, start, end, getPipe(dc.getKey()), diameter,
